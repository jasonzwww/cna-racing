...
export default async function StandingPage() {
    ...
    const standingsBySeries = standingsEntries.reduce(
        (acc, item) => {
            acc[item.series.key] = item.standings;
            return acc;
        },
        {} as Record<string, ReturnType<typeof computeDriverStandings>>
    );
    ...
    return (
        <main className="min-h-screen bg-zinc-950 text-zinc-100">
            <section className="mx-auto max-w-7xl px-6 py-12">
                ...
                <div className="mt-10">
                    <StandingsToggle seriesList={seriesCatalog} standingsBySeries={standingsBySeries} />
                </div>
            </section>
        </main>
    );
