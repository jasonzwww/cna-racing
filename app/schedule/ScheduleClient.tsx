...
const filters = [
    { id: "all", label: "全部", icon: "📋", color: "text-zinc-200" },
    { id: "gt3open", label: "GT3 Open", icon: "🏎️", color: "text-red-300" },
    { id: "rookie", label: "CNA 新手赛", icon: "⭐", color: "text-sky-300" },
];
...
export function ScheduleClient({ initialFilter = "all" }: ScheduleClientProps) {
    const [activeFilter, setActiveFilter] = useState<FilterId>(initialFilter);
    const [openMenu, setOpenMenu] = useState<string | null>(null);

    const seriesStyles: Record<
        Exclude<FilterId, "all">,
        { label: string; icon: string; accent: string }
    > = {
        gt3open: { label: "GT3 Open", icon: "🏎️", accent: "text-red-300" },
        rookie: { label: "CNA 新手赛", icon: "⭐", accent: "text-sky-300" },
    };
        ...
        <div className="flex flex-col gap-6">
            <div className="flex flex-wrap gap-2">
                {filters.map((filter) => (
                    ...
                    >
                        <span className={["inline-flex items-center gap-2", filter.color].join(" ")}>
            ...
            <div className="grid gap-4">
                {races.map((race) => {
                    const style = seriesStyles[race.seriesKey];
                            ...
                            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                                <div>
                                    <div className="text-xs tracking-[0.3em] text-zinc-400">
                                        <span className={["inline-flex items-center gap-2", style.accent].join(" ")}>
